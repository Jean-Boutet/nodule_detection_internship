...
class YoloLoss(nn.Module):
    def __init__(
        ...
        self.bce_obj = nn.BCEWithLogitsLoss(
            pos_weight=torch.tensor([obj_pos_weight]), reduction="mean"
        )
    ...
    def _match_targets(
        ...
        for s_idx, (stride, anchor_wh) in enumerate(zip(self.strides, self.anchors)):
            ...
            for b_i, t in enumerate(targets):
                ...
                wh = boxes[:, 2:] - boxes[:, :2]
                gx = (cxcy[:, 0] / stride).long().clamp_(0, grid - 1)
                gy = (cxcy[:, 1] / stride).long().clamp_(0, grid - 1)
                ...
                if mask.any():
                    idx_n, idx_a = torch.where(mask)
                    b_list.append(torch.full_like(idx_n, b_i))
                    a_list.append(idx_a)
                    gy_list.append(gy[idx_n])
                    gx_list.append(gx[idx_n])
                    tbox_list.append(boxes[idx_n])
                    tmal_list.append(mal[idx_n])
            if b_list:
                out.append((
