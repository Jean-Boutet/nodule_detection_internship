...
def _to_chw(arr: np.ndarray, channels: int) -> np.ndarray:
    if channels == 1:
        return _to_single_center_slice(arr)[None, ...]
    if channels == 3:
        return _to_chw_3(arr)
...
class LIDCDetectionDataset(Dataset):
    ...
    def __init__(
        ...
        self.col_image_path = col_image_path
        self.col_mask_path = col_mask_path
        self.central_only = bool(central_only)
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, Dict[str, Any]]:
        ...
        if self.central_only or self.in_channels == 1:
            image = _to_single_center_slice(arr)       # (H,W) float32
            image = np.expand_dims(image, 0)           # (1,H,W)
        else:
            image = _to_chw(arr, self.in_channels)
