...
class Conv(nn.Module):
    def __init__(self, ci: int, co: int, k: int = 3, s: int = 1, p: int | None = None, act: bool = True):
        ...
        self.conv = nn.Conv2d(ci, co, k, s, pad, bias=False)
...
class CSPDarknetLite(nn.Module):
    def __init__(self, in_ch: int = 3, base: int = 32):
        ...
        self.stem = Conv(in_ch, c1, k=3, s=2)
        self.stage2 = nn.Sequential(Conv(c1, c2, 3, 2), C3(c2, c2, n=1))
        self.stage3 = nn.Sequential(Conv(c2, c3, 3, 2), C3(c3, c3, n=2))
        self.stage4 = nn.Sequential(Conv(c3, c4, 3, 2), C3(c4, c4, n=2))
        self.stage5 = nn.Sequential(Conv(c4, c5, 3, 2), C3(c5, c5, n=1), SPPF(c5, c5))
...
class Yolo3DContext(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert backbone in ("cspdarknet_lite", "simple_3d_encoder")
        ...
        self.heads = nn.ModuleList([DetectHead(c, num_anchors_per_scale) for c in neck_ch])
